...
export function getQualificationStatus(advisor) {
  ...
  const qualified = bscOk && pdStatus === 'Met';
...
export function getSlabForRank(rank) {
  return SLAB_GRID.find(s => rank >= s.rankMin && rank <= s.rankMax)
    || SLAB_GRID[SLAB_GRID.length - 1];
}

export function getPayoutForRank(rank) {
